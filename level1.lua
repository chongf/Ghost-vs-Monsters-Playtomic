...
function new()
	...
	local callGameOver = function( isWin )
		...
		-- Playtomic: Log level score
		eventData = {
			type = "average",
			levelName = "level1", --temp solution, since we can't get levelName from inside the game engine :(
			value = gameScore,
		...
		local onGhostCollision = function( self, event )
			if event.phase == "began" then
				...
				elseif ghostObject.isHit then
					print("ghost lands @: " .. math.floor(self.x) .. ", " .. math.floor(self.y));

					-- Playtomic: Log coordinates where ghost lands
					eventData = {
						type = "heatmap",
						mapName = "level1",
						x = math.floor(self.x),
